...
func (h *JournalJSONHandler) TryHandle(d []byte) bool {
	if !bytes.Contains(d, []byte(`"__REALTIME_TIMESTAMP"`)) {
		return false
	}
...
func (h *JournalJSONHandler) UnmarshalJournalJSON(data []byte) error {
	...
	timestamp, ok := raw["__REALTIME_TIMESTAMP"]
	if ok {
		delete(raw, "__REALTIME_TIMESTAMP")
		timeString, ok := timestamp.(string)
		if !ok {
			return fmt.Errorf("__REALTIME_TIMESTAMP %v is not type string", timestamp)
		}
